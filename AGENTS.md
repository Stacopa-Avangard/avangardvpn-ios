...
- ⬜ **Export compliance is wrong in App Store Connect.** Both builds read `usesNonExemptEncryption: false`. This app is closed-source and ships its own crypto in `wireguard-go`, so the accurate answer is `true`. No rebuild needed — `BuildUpdateRequest` makes the field writable. Read [`EXPORT-COMPLIANCE.md`](EXPORT-COMPLIANCE.md) before touching it, and do not "fix" it by putting the key back in the plist.
